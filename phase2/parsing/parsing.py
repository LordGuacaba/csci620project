...
def parse_team_df_to_documents(df):
    """
    ...
    for index, row in df.iterrows():
        team = Team()
        team["_id"] = row["TEAM"]
        team["name"] = row["NICKNAME"]
        team["city"] = row["CITY"]
        team["first"] = row["FIRST"]
        team["last"] = row["LAST"]
        teams.append(team)
    return teams


def parse_player_df_to_documents(df):
    """
    ...
    for index, row in df.iterrows():
        player = Player()
        player["_id"] = row["PLAYERID"]
        player["firstname"] = row["FIRST"]
        player["lastname"] = row["LAST"]
        player["DOB"] = pd.to_datetime(row["BIRTHDATE"], errors="coerce")
        player["bats"] = row["BATS"]
        player["throws"] = row["THROWS"]
        players.append(player)
    return players


def parse_ballpark_df_to_documents(df):
    """
    ...
    for index, row in df.iterrows():
        ballpark = Ballpark()
        ballpark["_id"] = row["PARKID"]
        ballpark["name"] = row["NAME"]
        ballpark["city"] = row["CITY"]
        ballpark["state"] = row["STATE"]
