...
def parse_play_line(line: str, home_away: tuple) -> AtBat:
    ...
    # if line has a slash after error i.e. E9/TH, clean it up
    if re.search(r'(?<=E\d)/', line):
        line = clean_slashes(line)
    ...
    if len(play) > 1:
        ...
        play_no_slash = parts[6].split(".")
        atBat.setValue("play", play_no_slash[0] if play_no_slash[0] != "" else None)
